...
v2 (19/07) — THÊM BỘ LỌC ĐỒNG THUẬN: trong 5 nguồn (Polymarket, ECMWF,
GFS, ICON, UKMO), nếu >= 3 nguồn cùng chỉ vào ĐÚNG Ô mà mình định NO
(GFS luôn là 1 phiếu vì ô mục tiêu chính là ô GFS chọn) thì BỎ QUA không
vào lệnh — khi nhiều nguồn độc lập cùng đồng ý thì khả năng ô đó trúng
thật là cao, cược NO ngược đồng thuận là tự sát. Số liệu dẫn đến sửa:
tính đến 19/07 CD5 thắng 61.1% với giá NO trung bình ~65c (cần ~68% mới
hòa vốn) -> lỗ -23.79$.
...
# v2: cac cot du bao cua 4 model + Polymarket dung cho bo loc dong thuan
MODEL_COLS = ["fc_ecmwf_ifs025_c", "fc_gfs_seamless_c",
              "fc_icon_seamless_c", "fc_ukmo_seamless_c"]
CONSENSUS_MIN = 3  # >= 3/5 nguon cung chi vao o dinh NO -> bo qua


def count_consensus(s, pick_label, precision):
    """Dem so nguon (PM + 4 model) chi vao dung o pick_label."""
    votes = 0
    # Polymarket: o dam dong tin nhat
    if (s.get("pm_top_bucket") or "") == pick_label:
        votes += 1
    bd = P.parse_label(pick_label)
    if bd is None:
        return votes
    for col in MODEL_COLS:
        v = P.to_float(s.get(col))
        if v is None:
            continue
        native = C.c_to_f(v) if s.get("unit") == "F" else v
        if C.bucket_contains(bd, native, precision):
            votes += 1
    return votes
...
def enter(trades, snaps, full, now):
    ...
    for s in snaps:
        ...
        if not pick:
            continue
        # v2: bo loc dong thuan — >= 3/5 nguon cung chi vao o nay thi bo qua
        votes = count_consensus(s, pick["label"], s.get("precision") or "whole")
        if votes >= CONSENSUS_MIN:
            print(f"  [BO QUA] (CD5 v2) {s['city']} {s['target_date']} "
                  f"'{pick['label']}': {votes}/5 nguon dong thuan -> khong NO")
            continue
...
def main():
    ...
    print(f"\n[CHIEN DICH 5 v2 — NO theo GFS, loi >= $3, bo qua neu >=3/5 nguon dong thuan, $1000 ao]")
